...
```
python ~/analysis/analysis/experiment/train_model.py --config_path CONFIG_YAML_RELATIVE_PATH
```
### Embedding
...
```
python ~/analysis/analysis/experiment/eval_pretrained.py --config_path CONFIG_YAML_RELATIVE_PATH
``` -->

<!-- ### Transfer Learning
Additional scripts for training a downstream classification model (e.g. Logistic Regression or a Random Forest) trained on model generated embeddings as features are contained in `~/analysis/analysis/experiment/transfer_learning`. This transfer learning pipeline is distinct for hierarchical classification and our implementation uses the package `hiclass`. These can be run as follows:
```
python ~/analysis/analysis/experiment/transfer_learning/train_hiclass --config_path CONFIG_YAML_RELATIVE_PATH
```
for hierarchical multiclass classification.
```
python ~/analysis/analysis/experiment/transfer_learning/train_class --config_path CONFIG_YAML_RELATIVE_PATH
```
...
For a full detailed list, see documentation in ~/analysis/analysis/experiment/utils/config.py`
...
- Make sure `experiment_dir` is writable (currently hardcoded to `~/analysis/analysis/experiment` in the class).
