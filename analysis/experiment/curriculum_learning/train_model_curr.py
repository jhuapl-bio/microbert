...
def main(config: Config):
    ...
    # loop through data processor labels to get all or subset for model training labels
    ...
    # Assume: data_processor.labels is a list of label names in order
    indices_to_keep = []
    for label in config.labels:
    ...
    if len(config.labels) != len(data_processor.labels):
        ...
        logger.info("Filtering out labels from tokenized datasets")
        ...
        logger.info("Filtered testing data...")

        if not bool(config.train_iterable):
            logger.info("Filtered training data (non-iterable)...")
            tokenized_training_data = tokenized_training_data.map(
                filter_labels,
                fn_kwargs={"indices_to_keep": indices_to_keep}
            )    
        
    # load in trained model from model_trained_dir
    if config.model_trained_dir is None:
        raise ValueError(f"Must have valid trained model to load in for curriculum learning...{config.model_trained_dir} is not set")
    safetensors_file = os.path.join(config.model_trained_dir, 'model.safetensors')
    # Load safetensors weights
    state_dict = load_file(safetensors_file)

    # Remove classifier weights
    base_state_dict = {k: v for k, v in state_dict.items() if not k.startswith("classifiers")}
    
    # Count number of parameters
    num_base_params = sum(v.numel() for v in base_state_dict.values())
    logger.info(num_base_params)

    # model to be trained
    model = HierarchicalClassificationModel(config.base_model_name, num_labels, class_weights)
    ...
    custom_callbacks = [early_stopping_callback, epoch_timing_callback, log_metrics_callback, memory_debug_callback]
